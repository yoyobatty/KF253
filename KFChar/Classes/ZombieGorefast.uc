...
simulated function tick(float DeltaTime)
{
  Super.tick(DeltaTime);
     ...
     // If we're closeby our enemy...pour on a burst of raging zombie speed!!! Woooo!
     // Update. If he's got his head on, that is.

  if (Controller != none)
  {
   if (!bDecapitated)
   {
     if (Controller.Enemy != none && VSize(Controller.Enemy.location - location) <= 300 && !bRunning )
        StartRunning();

     if ( bRunning && VSize(Controller.Enemy.location - location) > 300 || Controller.Enemy == none)
      StopRunning();
   }
  } 
}
...
simulated function StartRunning()
{
