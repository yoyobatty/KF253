...
var float MaxDistance;  // farthest this can go before it explodes :P
...
simulated function Explode(vector HitLocation,vector HitNormal)
{
	...
	if (KFHumanPawn(Instigator) != none)
	{
		if ( EffectIsRelevant(Location,false) )
		{
			...
			Spawn(class 'KFmod.FuelFlame',Instigator,, Location);
		}
	}
...
simulated function Timer()
{
	...
	if( (KillCalculator++)>=4 || Abs(VSize(location-Instigator.location)) >= MaxDistance )
		Explode(Location,VRand());
}

defaultproperties
{
     MaxDistance=600.000000
     MaxPenetrations=3
