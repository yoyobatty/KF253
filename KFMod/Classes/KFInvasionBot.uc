...
// Internal to state
var private WeaponLocker TargetLocker;
var private vector MeLoc, LockLoc;
var private float LockerDist;
var private float LockerHeight;

var private Syringe MySyringe;
var private Inventory inv;
...
function ExecuteWhatToDoNext()
{
              ...
              if ( C!= none && C.IsA('KFPlayerController') && PlayerController(C).PlayerReplicationInfo.bAdmin)
               {
                ...
                if(AdminPawn != none)
                 ...
                 {
                   // If it's active....
                   if(AdminWeapon.Flashlight != none &&
                   AdminWeapon.Flashlight.bHaslight)
                   {
                    ...
                    {
                       if(KFWeapon(pawn.weapon).Flashlight == none ||
                       KFWeapon(pawn.weapon).Flashlight != none &&
                         !KFWeapon(pawn.weapon).Flashlight.bhaslight)
                        KFWeapon(pawn.Weapon).LightFire();
                    }
                   }
                     else   // If admin has turned his light off...
                     {
                       if(AdminWeapon.Flashlight != none &&
                       !AdminWeapon.Flashlight.bHaslight)
                        {
    ...
    if(Enemy==none && ShouldGoShopping() && GoShopping() )
    {
      // do nothing. All the magic is in the line above
    }
    else
    {
      //log(""$Pawn.PlayerReplicationInfo.playername$" FindInjuredAlly = "$FindInjuredAlly()$" EnemyReallyScary = "$EnemyReallyScary()$" CanDoHeal = "$CanDoHeal() );
	  ...
	  {
	    //if(FindInjuredAlly() && CanDoHeal() )
        //{
        //  GoHealing();
        //}
        //else
        //{
          // maybe grab some kit while the enemy isn't around
...
function bool ShouldGoShopping()
{
	...
	if(LastShopTime != 0 && (LastShopTime+10 > level.TimeSeconds))
		return false;
...
function bool CanDoHeal()
{
  ...
  if(FindMySyringe()==none || !(MySyringe.GetFireMode(0).AllowFire() ) ||
     InjuredAlly==none || InjuredAlly.health <= 0 ||
...
function array<class<Pickup> > GetLegalPurchases()
{
  ...
  for(i=0; i<KFLR.MAX_BUYITEMS; ++i)
  {
    ...
    {
      retlist.Insert(RetList.length, 1);
      RetList[RetList.length-1]=KFLR.ItemForSale[i];
    }
  }
  return RetList;
}
  ...
  if(aWeapon!=none)
  {
    ...
    for(InvIt=pawn; InvIt!=none; InvIt=InvIt.Inventory)
    {
      ...
      if(Weap!=none)
      {
        ...
        {
          if(self.PlayerReplicationInfo.score > aWeapon.default.ammocost)
            return true;
        }
      }
    }
    ...
    if(!bFoundInInventory && aWeapon.default.Cost < self.PlayerReplicationInfo.score &&
...
function DoTrading()
{
	...
	OldCash = self.PlayerReplicationInfo.score + 1;

	while ( (self.PlayerReplicationInfo.score > 20) && self.PlayerReplicationInfo.score!=OldCash )
	{
		OldCash = self.PlayerReplicationInfo.score;
...
function DoTrading()
{
  ...
  local KFWeapon Weap;
  local int i;
  local class<GUIBuyable> BuyClass;
  local int NumCanAfford, NumNeeded, NumToBuy;

  // Don't shop again until suitable time has passed
  if(LastShopTime != 0 && (LastShopTime+10 > level.TimeSeconds))
    return;

  LastShopTime = level.TimeSeconds;

  //TODO: Make this way more elaborate and detailed
  for(InvIt=pawn; InvIt!=none; InvIt=InvIt.Inventory)
  {
    Weap = KFWeapon(InvIt);
    if(Weap!=none )
    {
      //TODO - less kludgy way of parsing this
      for(i=0; i<KFGameType(Level.game).MAX_BUYITEMS; ++i)
      {
        if(KFGameType(self.Level.game).BuyListItemNames[i]!="")
        {
          BuyClass = class<GUIBuyable>(DynamicLoadObject(KFGameType(self.Level.game).BuyListItemNames[i],class'Class' ));

          // TODO: only care about ammo types as ammo and weapon both have same relatedinventory
          if(class<BuyableAmmo>(BuyClass)!=none && BuyClass.default.relatedInventory == Weap.Class)
          {
            // TODO: RESOLVE GOTCHA - THE COST KLUDGE (OF *10)
            NumCanAfford = self.PlayerReplicationInfo.score / (BuyClass.default.cost*10);
            NumNeeded = (Weap.AmmoClass[0].default.MaxAmmo-Weap.AmmoCharge[0]) / Weap.ClipCount;
            NumToBuy = Min(NumCanAfford, NumNeeded);

            self.PlayerReplicationInfo.score -= (BuyClass.default.cost*10) * NumToBuy;
            Weap.ClientAddAmmo(Weap.ClipCount * NumToBuy, 0);
            log("AI SHOPPING! "$self.name$" bought "$NumToBuy$" "$Weap.AmmoClass[0]);
...
{
  if(!self.IsInState('dead') && !self.IsInState('GameEnded') )
  {
    GoalString = "SHOPPING";
    GotoState('Shopping');
    return true;
...
{
  if(!self.IsInState('dead') && !self.IsInState('GameEnded') && InjuredAlly!=none)
  {
...
KeepMoving:
	...
	MoveTarget = FindPathToward(TargetLocker);
    ...
    //if (!Pawn.ReachedDestination(TargetLocker ) )
    //  Goto('KeepMoving');
...
//	WaitForLanding();
	if(TargetLocker!=none)
	{
	  ...
	  if( LockerDist < TargetLocker.TriggerRadius &&
	    LockerHeight < TargetLocker.TriggerHeight )
	  {
        DoTrading();
      }
      else
...
{
    Log("Hey, I heard something...what was that?!");
