...
var float NextStatsUdpTime,EndGameHUDTime;
...
function DrawCustomBeacon(Canvas C, Pawn P, float ScreenLocX, float ScreenLocY)
{
	...
	if( KFPlayerReplicationInfo(P.PlayerReplicationInfo)!=None )
	{
		...
		if( VE!=None )
		{
			...
			MW *= VeterancyMatScaleFactor;
			MH = MW;

			if( M!=None )
			{
				C.SetPos(ScreenLocX - FClamp(XL/2,BarLength/2,500.f)-50,ScreenLocY-YL-18);    // -31
				C.DrawTile(M,MW,MH,0,0,M.MaterialUSize(),M.MaterialVSize());
			}
...
simulated function DrawHud( Canvas C )
{
	...
	//	if (CurrentGame!= none && CurrentGame.bEnemyHealthBars)
			DrawEnemyHealthBars(C);
	}
	PassStyle = STY_Modulated;
	DrawModOverlay(C);
	if (KFPlayerReplicationInfo(PlayerOwner.PlayerReplicationInfo).bViewingMatineeCinematic)
...
simulated function UpdateHud()
{
	...
	DigitsClipsLeft.Value=CurClipsPrimary;
        ...
        /*
        // Flash Health bar, if hurt.
	if (pawnowner.health <= (0.25 * pawnowner.healthmax))
		HealthMeter.WidgetTexture = MaterialSequence 'KFKillMeNow.HUDIconsCritical';
	else if (KFPawn(pawnowner).healthToGive > 0)
			HealthMeter.WidgetTexture = TexOscillator 'KFKillMeNow.HUDIconsOSC';
	else HealthMeter.WidgetTexture = default.HealthMeter.WidgetTexture;

	
	// Flash boxes if we're 3/4 full or more.
		if( KFHumanPawn(Pawnowner).CurrentWeight >= (0.75*KFHumanPawn(Pawnowner).MaxCarryWeight) && CarryMeter[i].Tints[0].R == WEIGHTON_R )
			CarryMeter[i].WidgetTexture = MaterialSequence 'KFKillMeNow.HUDIconsCritical';
		else CarryMeter[i].WidgetTexture = default.CarryMeter[i].WidgetTexture;
         */ 		
	...
	if (KFPawn(pawnowner).BileCount > 0)
	{
	...
	if (VisionOverlay != none )
	{
		...
		// If critical, pulsate.  otherwise, dont.
              ...
              if(PlayerOwner.pawn != none)
              {
                ...
                if (KFPawn(PlayerOwner.pawn).BurnDown > 0)
		...
		// Dead / Spectating Players see Red (except for normal spectators!)
...
// Draw Enemy Health Bars (if requested).
simulated function DrawEnemyHealthBars(Canvas C)
{
	local KFMonster KFEnemy;

	foreach DynamicActors(class'KFMonster',KFEnemy)
	{
             if(PlayerOwner.pawn != none && 
             KFEnemy != none)
             {
                if (KFEnemy.Health > 0 && !KFEnemy.Cloaked() &&
		KFPawn(PlayerOwner.pawn).GetVeteran().Static.ShowBars())
...
function DisplayHit(vector HitDir, int Damage, class<DamageType> damageType)
{
	...
	if(class<DamTypeZombieAttack>(HUDHitDamage)!=none)
		DamageTime[0]= class<DamTypeZombieAttack>(HUDHitDamage).default.HUDTime;
	else DamageTime[0] = Clamp(Damage,20,30);
}
...
function DrawDamageIndicators(Canvas C)
{
	...
	if ( DamageTime[0] > 0 )
	{
		...
		Alpha = clamp(DamageTime[0]*2, 0, 200);
...
simulated function DrawWeaponName(Canvas C)
{
	...
	{
		WeaponDrawTimer = Level.TimeSeconds+9999;
...
simulated function DrawEndGameHUD( Canvas C, bool bVictory )
{
	...
	if( EndGameHUDTime>=1 )
		C.DrawColor.A = 255;
	else C.DrawColor.A = (EndGameHUDTime*255.f);
	C.DrawColor.R = 255;
	C.DrawColor.G = 255;
	C.DrawColor.B = 255;
	...
	C.Style = ERenderStyle.STY_Alpha;
	if( bVictory )
		C.DrawTile( Material'VictoryShader', Scalar, Scalar, 0, 0, 1024, 1024);
	else C.DrawTile( Material'DefeatShader', Scalar, Scalar, 0, 0, 1024, 1024);
}
...
defaultproperties
{
     ...
     TorchBatteryIcon=(WidgetTexture=Texture'KFKillMeNow.KFHUDIcons',RenderStyle=STY_Alpha,TextureCoords=(X1=14,Y1=180,X2=30,Y2=192),TextureScale=0.500000,DrawPivot=DP_LowerLeft,PosY=1.000000,OffsetX=10,OffsetY=-60,Scale=1.000000,Tints[0]=(B=255,G=255,R=255,A=255),Tints[1]=(B=255,G=255,R=255,A=255))
     ...
     BatteryMeter=(WidgetTexture=Texture'KFKillMeNow.KFHUDIcons',RenderStyle=STY_Alpha,TextureCoords=(X1=41,Y1=150,X2=250,Y2=163),TextureScale=0.500000,DrawPivot=DP_LowerLeft,PosY=1.000000,OffsetX=37,OffsetY=-60,ScaleMode=SM_Right,Scale=1.000000,Tints[0]=(B=255,G=255,R=255,A=255),Tints[1]=(B=255,G=255,R=255,A=255))
