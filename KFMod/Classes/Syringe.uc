...
simulated function bool StartFire(int Mode)
{
	...
	// Prevent firing while the other mode's animation is still active
	OtherMode = 1 - Mode;
	if (FireMode[OtherMode] != None && FireMode[OtherMode].NextFireTime > Level.TimeSeconds)
		return false;
...
simulated function Tick(float dt)
{
	if ( Level.NetMode!=NM_Client && AmmoCharge[0]<MaxAmmoCount && RegenTimer<Level.TimeSeconds )
	{
		...
		{
			AmmoCharge[0] += 10 * KFPlayerReplicationInfo(Instigator.PlayerReplicationInfo).ClientVeteranSkill.Static.GetSyringeChargeRate();
		}
		else
		{
			AmmoCharge[0] += 10;
